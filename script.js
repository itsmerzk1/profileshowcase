...
function applyProfileVisuals(m, p){
  const avatar = discordAvatarUrl(p, m.avatar || `https://api.dicebear.com/9.x/initials/svg?seed=${encodeURIComponent(m.name)}`);
...
/* TEAM OKD v3 Visitor Analytics - owner only dashboard
   Security mode: stores visitor IP + approximate location in Firestore.
   Use responsibly and add a privacy notice for public visitors. */
